...
def crop(data, in_file_path, out_file_path, rewrite=False, out_size=32):
    ...
    resized_cropped_img.save(out_file_path)
    return 1


def process_file(file_name, rewrite=False):
    ...
    with open(file_path, 'r') as csvfile:
        ...
        for d in datareader:
            ...
            pic_written = crop(d[1:], in_file_path, out_file_path, rewrite)
            written_pics_per_file = written_pics_per_file + pic_written

    return written_pics_per_file

def generate_selections(rewrite=False):
    written_files = 0
    ...
    for f in image_selection_files:
        file_name = f.split(".")[0]
        written_pics_per_file = process_file(file_name, rewrite)
